namespace Torshia.Services
{
    ...
    public class TasksService : BaseService, ITasksService
    {
        ...
        public ICollection<TaskViewModel> AllNonReported()
        {
            ...
            var tasks = this.context.Tasks.Where(t => t.IsReported == false);
        ...
        {
            var sectors = this.context.TasksSectors.Where(ts => ts.TaskId == taskId).Select(ts => ts.Sector).ToList();
        ...
        private void FillAffectedSectors(ICollection<string> affectedSectors, string taskTitle)
        {
            var taskId = this.context.Tasks.FirstOrDefault(t => t.Title == taskTitle).Id;

            foreach (var affectedSector in affectedSectors)
            {
                this.context.TasksSectors.Add(
                    new TaskSector
                        {
